...
class BlogUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        try:
            if attrs["home_page"]:
                if Blog.objects.filter(home_page=True).count() >= 3:
                    raise serializers.ValidationError("Home Page has the maximum limit of Blogs.")
            elif attrs["press_center"]:
                if Blog.objects.filter(press_center=True).count() >= 3:
...
class TestimonialsListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Testimonials
        fields = ["id", "name", "photo", "youtube_link", "video_id", "company"]
...
class TestimonialsDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Testimonials
        exclude = ["id", "created_at", "updated_at", "press_center", "company"]
