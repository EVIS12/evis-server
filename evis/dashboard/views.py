...
class ExhibitorViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = ExhibitorUpdateSerializer
    filter_backends = [
        DjangoFilterBackend,
    ]
...
class ContractFileViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":
            ...
            return ContractFileUpdateSerializer
...
class ContractFormViewSet(viewsets.ModelViewSet):
    ...
    def download_by_filter(self, request):
        ...
        headers = ["Name", "Company", "Email", "Country", "Phone", "Industry"]
...
class PartnerAndSponserViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":
            ...
            return PartnerAndSponserCUSerializer
        return PartnerAndSponserDetailSerializer


# Category Partners and Sponsers
class CategoryPartnerAndSponserViewSet(viewsets.ModelViewSet):
    queryset = Categorty.objects.all().order_by("order_num")
...
class AttendeesViewSet(viewsets.ModelViewSet):
    queryset = Attendees.objects.all().order_by("name")
...
# Awards (Student Project)
# ----------------------------------------------------------------
class StudentProjectViewSet(mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    queryset = StudentProject.objects.all()[:20]
    ...
    def export_to_excel(self, request):
        ...
        response = HttpResponse(content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        response["Content-Disposition"] = 'attachment; filename="student_projects.xlsx"'
        ...
        for student_project in student_projects:
            data_row = [
                student_project.project_status,
                student_project.team_number,
                student_project.university,
                student_project.faculty,
                student_project.project_name,
                student_project.team_come_with,
                student_project.project_abstract,
                student_project.team_leader_name,
                student_project.team_leader_email,
                student_project.team_leader_phone,
                student_project.team_leader_social_media,
            ]
            ws.append(data_row)
