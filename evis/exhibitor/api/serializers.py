...
class ExhibitorCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if attrs["is_partner"] and Exhibitor.objects.filter(is_partner=True).count() >= 25:
            raise serializers.ValidationError("Exhibitor has the maximum limit of Partners.")
        elif attrs["is_sponser"] and Exhibitor.objects.filter(is_sponser=True).count() >= 25:
            raise serializers.ValidationError("Exhibitor has the maximum limit of Sponsers.")
        elif attrs["slider"] and Exhibitor.objects.filter(slider=True).count() >= 15:
...
class ExhibitorUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        try:
            ...
            pass
        try:
            if attrs["is_sponser"] and Exhibitor.objects.filter(is_sponser=True).count() >= 25:
                raise serializers.ValidationError("Exhibitor has the maximum limit of Sponsers.")
        except KeyError:
            pass
        try:
            if attrs["slider"] and Exhibitor.objects.filter(slider=True).count() >= 15:
                raise serializers.ValidationError("Exhibitor has the maximum limit of Slider.")
        except KeyError:
            pass
        return attrs
