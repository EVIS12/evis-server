...
def activate_account(request, uid, token):
    ...
    DOMAIN = getattr(settings, "DOMAIN") or "server.evinnovationsummit.com"
    return render(
        request,
        "accounts/activate_account.html",
        {"uid": uid, "token": token, "url": f"{PROTOCOL}://{DOMAIN}/api/v1/accounts/api/v1/users/activation/"},
    )
...
def reset_password_confirm(request, uid, token):
    ...
    return render(
        ...
        "accounts/reset_password_confirm.html",
        {
            "uid": uid,
            "token": token,
            "reset_pass_confirm_url": f"{PROTOCOL}://{DOMAIN}/api/v1/accounts/api/v1/users/reset_password_confirm/",
        },
    )
